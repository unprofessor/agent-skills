...
disturbing the worker. Also formalize the tech-lead re-dispatch technique:
...
enforcement. The tech lead also pre-seeded the approach-B conclusion
...
- [ ] PROCESS.md adds a "Re-dispatching an interrupted worker" note (edge
  cases or execution): the tech lead inspects `resume.sh` output, writes
