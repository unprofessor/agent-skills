...
Add a `scripts/backlinks.sh` utility that finds all `.plan/` files wiki-linking to a given slug. Currently the skill documents `grep -rn '\[\[slug\]' .plan/` as the manual method — this wraps that in a proper script with `PLAN_DIR` support.
...
Existing patterns to follow: `review.sh` for env var handling (`PLAN_DIR`, `PLAN_TRUNK`), `lint.sh` for wiki-link extraction regex (`grep -oE '\[\[[^]]+\]\]'` and stripping `|alias`/`#heading` suffixes).
...
- [ ] Honors `PLAN_DIR` env var (default `.plan`)
