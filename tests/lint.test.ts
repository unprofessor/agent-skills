...
function t(opts: Partial<ParsedTicket> & { id: string; kind: string }): ParsedTicket {
	return {
		id: opts.id,
...
describe("checkBacklog", () => {
	...
	it("detects missing id", () => {
		...
		const report = checkBacklog([
			input(".plan/epics/01-v1.md", ticket),
		]);
		expect(report.errorCount).toBeGreaterThanOrEqual(1);
		expect(report.issues.some((i) => i.message.includes("missing id"))).toBe(true);
	});
	...
	it("detects mismatched id vs filename slug", () => {
		...
		expect(report.errorCount).toBeGreaterThanOrEqual(1);
		expect(report.issues.some((i) => i.message.includes("does not match filename slug"))).toBe(true);
	});
	...
	it("detects kind/dir mismatch", () => {
		...
		expect(report.errorCount).toBeGreaterThanOrEqual(1);
		expect(report.issues.some((i) => i.message.includes("but the file lives in the epics directory"))).toBe(true);
	});
	...
	it("detects invalid status", () => {
		...
		expect(report.errorCount).toBeGreaterThanOrEqual(1);
		expect(report.issues.some((i) => i.message.includes("invalid status"))).toBe(true);
	});
	...
	it("detects duplicate slug", () => {
		...
		expect(report.errorCount).toBeGreaterThanOrEqual(1);
		expect(report.issues.some((i) => i.message.includes("duplicate slug"))).toBe(true);
	});
	...
	it("detects epic with parent", () => {
		...
		expect(report.errorCount).toBeGreaterThanOrEqual(1);
		expect(report.issues.some((i) => i.message.includes("epics must not have a parent"))).toBe(true);
	});

	it("detects story/task without parent", () => {
		const ticket = t({ id: "net", kind: "story" });
		const report = checkBacklog([
			input(".plan/stories/01-net.md", ticket),
		]);
		expect(report.errorCount).toBeGreaterThanOrEqual(1);
		expect(report.issues.some((i) => i.message.includes("must name a parent slug"))).toBe(true);
	});

	it("detects dangling parent", () => {
		const ticket = t({ id: "proxy", kind: "task", parent: "ghost" });
		const report = checkBacklog([
			input(".plan/tasks/01-proxy.md", ticket),
		]);
		expect(report.errorCount).toBeGreaterThanOrEqual(1);
		expect(report.issues.some((i) => i.message.includes("parent 'ghost' does not exist"))).toBe(true);
	});
	...
	it("warns on wrong-kind parent", () => {
		...
		expect(report.issues.some((i) => i.message.includes("usually a story"))).toBe(true);
		expect(report.errorCount).toBe(0);
	});

	it("detects dangling depends_on", () => {
		const epic = t({ id: "v1", kind: "epic" });
		const task1 = t({ id: "a", kind: "task", parent: "v1", depends_on: ["ghost-task"] });
		const report = checkBacklog([
			input(".plan/epics/01-v1.md", epic),
			input(".plan/tasks/01-a.md", task1),
		]);
		expect(report.errorCount).toBeGreaterThanOrEqual(1);
		expect(report.issues.some((i) => i.message.includes("depends_on 'ghost-task' does not exist"))).toBe(true);
	});
	...
	it("detects self-dependency (not as cycle)", () => {
		...
		expect(report.errorCount).toBeGreaterThanOrEqual(1);
		expect(report.issues.some((i) => i.message === "depends_on itself")).toBe(true);
		// Must NOT report a cycle for self-dep
		expect(report.issues.some((i) => i.message.includes("depends_on cycle"))).toBe(false);
	});
	...
	it("detects dependency cycle", () => {
		...
		expect(report.errorCount).toBeGreaterThanOrEqual(1);
		expect(report.issues.some((i) => i.message.includes("depends_on cycle"))).toBe(true);
	});
	...
	it("warns on unresolved wiki-link", () => {
		...
		expect(report.warningCount).toBeGreaterThanOrEqual(1);
		expect(report.issues.some((i) => i.message.includes("[[no-such-note]] matches no ticket slug"))).toBe(true);
		expect(report.errorCount).toBe(0);
	});
	...
	it("does not error on block-style depends_on (parsed correctly)", () => {
		...
		expect(report.issues.some((i) => i.message.includes("depends_on 'a' does not exist"))).toBe(false);
		expect(report.errorCount).toBe(0);
	});
	...
	it("reports status 'blocked' as valid", () => {
		...
		expect(report.issues.some((i) => i.message.includes("invalid status"))).toBe(false);
	});
